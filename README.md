...
|-------|--------------------|----------------|
| Idéation | R-0 (Albert Business Analyst) | gemma3:12b → Gemini → Opus |
| Architecture | R-2 (Albert System Architect) | gemma3:12b → Gemini → Opus |
| Sprint Planning | R-3 (Albert Scrum Master) | qwen2.5-coder:7b → Gemini → Sonnet |
| Développement | R-4 (Albert Dev Team) | qwen2.5-coder:7b → Gemini → Sonnet |
| Versioning | R-5 (Albert Release Manager) | qwen2.5-coder:7b → Gemini → Sonnet |
| Qualité | R-6 (Albert QA & DevOps) | qwen2.5-coder:7b → Gemini → Sonnet |

**Nghia reste :** R-1 (Nghia Product Owner) + R-7 (Nghia Stakeholder) — la vision et le Go/No-Go final.
...
Albert ne fait rien sans validation explicite de Nghia aux jalons critiques (H1 (validation Gros Ticket)–H6 (résolution conflit Git) : Gros Ticket, Architecture, Sprint Backlog, Sprint Review, escalade API, conflit Git). Tout est tracé dans Git, LangSmith, et les ADRs.
...
PHASE 0 — IDÉATION (hors Agile, gratuite)
  Nghia + Albert R-0 (Albert Business Analyst) → itérations libres → Gros Ticket mature
  ↓ H1 (validation Gros Ticket) : Nghia valide

CYCLE AGILE (sprints itératifs)
  E1 (idéation) → Product Backlog         (Nghia / R-1 (Nghia Product Owner))
  E2 (architecture) → Architecture + DoD  (Albert R-2 (Albert System Architect)) ← H2 (validation Architecture) : Nghia valide
  E3 (Sprint Backlog) → Sprint Backlog    (Albert R-3 (Albert Scrum Master)) ← H3 (validation Sprint Backlog) : Nghia valide
  E4 (exécution code) → Code + Git        (Albert R-4 (Albert Dev Team) + R-5 (Albert Release Manager))
  E5 (tests CI) → Tests + CI              (Albert R-6 (Albert QA & DevOps))  ← H4 (Sprint Review) : Nghia valide
  E6 (clôture sprint) → Retrofit + Release (Albert R-5 (Albert Release Manager))
